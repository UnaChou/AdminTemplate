<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PermissionRole.aspx.cs" Inherits="AdminTemplate.AdminSystem.PermissionRole" %>
            ...
            <td>
                <table border="0" width="100%" cellspacing="0" cellpadding="0">
                    <tr>
                        ...
                        <td align="left" style="padding-left: 10px">目錄：系統設定 ► 角色權限設定
                        </td>
                        <td align="right">
                            <!-- Function Menu -->
                            <span class="ui-tab ui-widget ui-state-default ui-corner-bottom" onmouseover="$(this).addClass('ui-state-hover');" onmouseout="$(this).removeClass('ui-state-hover');">
                                <a href="PermissionAccount.aspx">帳號權限設定</a></span>
                            <span class="ui-tab ui-widget ui-state-default ui-corner-bottom ui-state-active" onmouseover="$(this).addClass('ui-state-hover');" onmouseout="$(this).removeClass('ui-state-hover');">
                                <a href="javascript://">角色權限設定</a></span>
                        </td>
                    </tr>
                </table>
                <hr />
                ...
                <div id="divNodeData" style="width: 90%;">
                    <asp:Repeater ID="rptNodeData" runat="server">
                        <HeaderTemplate>
                            <table gid="tblNodeData" width="100%">
                                <tr>
                                    ...
                                    <th style="width: 50px">瀏覽
                                    </th>
                                    <th style="width: 50px">新增
                                    </th>
                                    <th style="width: 50px">修改
                                    </th>
                                    <th style="width: 50px">刪除
                                    </th>
                        ...
                        <ItemTemplate>
                            <tr>
                                <td align="left">
                                    <%#Eval("ParentID") == "0"? "<span class='ui-icon ui-icon-folder-open' style='display: inline-block'></span>":""%>
                                    <%#Eval("NodeName")%>
                                </td>
                                <td align="center">
                                    <span class="<%#Eval("CanBrowse").ToString() == "True"? "ui-icon ui-icon-check": "ui-icon ui-icon-close ui-state-disabled"%>" style="display: inline-block"></span>
                                </td>
                                <td align="center">
                                    <span class="<%#Eval("CanInsert").ToString() == "True"? "ui-icon ui-icon-check":"ui-icon ui-icon-close ui-state-disabled"%>" style="display: inline-block"></span>
                                </td>
                                <td align="center">
                                    <span class="<%#Eval("CanModify").ToString() == "True"? "ui-icon ui-icon-check":"ui-icon ui-icon-close ui-state-disabled"%>" style="display: inline-block"></span>
                                </td>
                                <td align="center">
                                    <span class="<%#Eval("CanDelete").ToString() == "True"? "ui-icon ui-icon-check": "ui-icon ui-icon-close ui-state-disabled"%>" style="display: inline-block"></span>
                                </td>
                                <td align="center">
                                    ...
                                    <button id="btnBrowse_<%#Eval("ParentID")%>_<%#Eval("NodeID")%>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" onclick="return QuickUpdate(this);">
                                        <span class="ui-button-text">僅瀏覽</span></button>
                                    ...
                                    <button id="btnAllFn_<%#Eval("ParentID")%>_<%#Eval("NodeID")%>" class="ui-button ui-button-text-only ui-widget ui-state-default ui-corner-all" onclick="return QuickUpdate(this);">
                                        <span class="ui-button-text">全功能</span></button><%#Eval("ParentID") == "0"? "</span>": ""%>
                                    <button id="btnEdit_<%#Eval("ParentID")%>_<%#Eval("NodeID")%>" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" onclick="return RowCommand(this);" pid="<%#Eval("ParentID")%>" nodename="<%#Eval("NodeName")%>" permission="<%#Eval("CanBrowse")%>_<%#Eval("CanInsert")%>_<%#Eval("CanModify")%>_<%#Eval("CanDelete")%>">
                                        <span class="ui-button-icon-primary ui-icon ui-icon-pencil"></span>
                                        <span class="ui-button-text">編輯</span></button><% }%>
                                </td>
                            </tr>
                        </ItemTemplate>
                        <FooterTemplate>
                    ...
                    <td>
                        <table border="0" cellspacing="0" cellpadding="5">
                            <tr>
                                ...
                                <td id="frmPermission_RoleName"></td>
                            </tr>
                            <tr>
                                <td align="right">節點名稱：&nbsp;
                                </td>
                                <td id="frmPermission_NodeName"></td>
                            </tr>
                            <tr>
                                <td align="right">權限設定：&nbsp;
                                </td>
   ...
   { %>
    ...
    <% } %>
        ...
        function WidgetRefresh() {
            ...
            //    .find('button[gid=btnEdit]').button({ icons: { primary: 'ui-icon-pencil'} }).click(RowCommand).end()
            //    .find('button[gid=btnBrowse]').button().click(QuickUpdate).end()
            //    .find('button[gid=btnAllFn]').button().click(QuickUpdate);
        }

        var SearchParams = $('#ddlRoleList').val() + '_' + $('#ddlNodeList').val();
        function GetNodeData() {
            $('#frmPermission_RoleName').html($('#ddlRoleList option:selected').text());
            SearchParams = $('#ddlRoleList').val() + '_' + $('#ddlNodeList').val();
            $('#divNodeData').hide();

            $.ajax({
                type: 'POST',
                url: location.pathname.substring(location.pathname.lastIndexOf('/') + 1),
                data: 'fnMode=search&ParentID=' + $('#ddlNodeList').val() + '&RoleID=' + $('#ddlRoleList').val(),
                error: onError,
                success: function (msg) {
                    ...
                    if (msg.toString().indexOf('err:') == 0) {
                        ...
                        if (msg == 'Empty') { $('#divNodeData').html(Tip('highlight', 'info', '角色已具備選取節點全部權限')).show(); return; }
                        var obj = $(msg);
